...
import { ArrowLeft, LineChart as LineChartIcon, Landmark, Layers, Coins, Clock, BarChart3, ArrowLeftRight, Eye, ChevronDown, Search } from 'lucide-react';
    ...
    <div ref={containerRef} style={{ position: 'relative', width: '100px' }}>
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Список отфильтрованных месяцев */}
...
export default function GraphsPage() {
  ...
  // Валидация: Если конечная дата меньше начальной, принудительно подтягиваем её к startMonth
  ...
  activeCurrencies.forEach(c => {
    ...
    if (firstValidSnap) {
      if (Number(firstValidSnap.data.rates[c]) < 0.1) invertedCurrencies.add(c);
    }
  });
  ...
  filteredSnapshots.forEach(s => {
    s.data.organizations.forEach(org => {
      org.balances.forEach(b => {
        if (b.currency && Number(b.amount) > 0) usedCurrenciesSet.add(b.currency);
      });
    });
  });
    ...
    s.data.organizations.forEach(org => {
      ...
      org.balances.forEach(b => {
        ...
        if (b.currency === baseCurrency) {
          ...
          const rate = Number(s.data.rates[b.currency] || 0);
    ...
    s.data.organizations.forEach(org => {
      org.balances.forEach(b => {
        ...
        if (b.currency === baseCurrency) {
          ...
          const rate = Number(s.data.rates[b.currency] || 0);
          valInBase = amount * rate;
        }
        point[b.currency] += Math.round(valInBase);
      });
    });
    return point;
  });
    ...
    if (globalIdx > 0) {
      ...
      s.data.organizations.forEach(org => {
        org.balances.forEach(b => {
          ...
          if (amt === 0) return;
          currentCurrencies[b.currency] = (currentCurrencies[b.currency] || 0) + amt;
        });
      });
      ...
      prevSnapshot.data.organizations.forEach(org => {
        org.balances.forEach(b => {
          const amt = Number(b.amount || 0);
          if (amt === 0) return;
          prevCurrencies[b.currency] = (prevCurrencies[b.currency] || 0) + amt;
        });
      });
      ...
      s.data.organizations.forEach(org => {
        org.balances.forEach(b => {
          const amt = Number(b.amount || 0);
          const rate = b.currency === baseCurrency ? 1 : Number(s.data.rates?.[b.currency] || 0);
          organicDelta += amt * rate;
        });
      });
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div className="flex justify-between items-center bg-[rgba(255,255,255,0.01)]" style={{ borderBottom: '1px solid var(--glass-border)', paddingBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {/* Перенесенная и обновленная кнопка Show Hidden */}
          {isAnythingHidden && (
            <button
          ...
          {/* Timeframe панель на базе SearchableSelect */}
          <div style={{
            display: 'flex',
            alignItems: 'center',
            gap: '8px',
            ...
            <SearchableSelect
              ...
              options={availableMonths.filter(m => !startMonth || m >= startMonth)}
              placeholder="End"
            />
          </div>
        </div>
      ...
      {/* СЕКЦИЯ 2: UX & OPERATIONAL METRICS */}
      <div>
        <h3 className="mb-4" style={{ color: 'var(--accent)', fontSize: '13px', fontWeight: 600, letterSpacing: '0.05em' }}>UX & OPERATIONAL METRICS</h3>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
